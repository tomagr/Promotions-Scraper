...
class BaseInteractor < Interactor
	...
	def invalid_title title
		invalid :title, 'Title cant be blank' if title.blank?
	end

	def invalid_status status
		invalid :status, 'Status cant be blank' if status.blank?
	end

	def invalid_site_id site_id
		invalid :site_id, 'site_id cant be blank' if site_id.blank?
	end
